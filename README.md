...
| Web Panel — full hosting UI | ✅ every screen wired to a **real backend** + typed API client: sites, domains/DNS (+ nameservers), SSL, databases, email + webmail + DKIM, FTP, file manager, cron, backups (+ schedules), runtime, one-click apps, metrics, health, logs, firewall, WAF, audit, API tokens + passkeys, notifications, plus the commercial layer (reseller, white-label, webhooks, billing, migrations) gated by edition |
