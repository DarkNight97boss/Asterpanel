...
| WebAuthn/Passkeys | ✅ full ceremony on `go-webauthn`: register + login `begin`/`finish` (challenge in Redis), credentials stored in `webauthn_credentials`, sign-count tracked. UI: passkey management on the API-Tokens page + "Sign in with a passkey" on login. *(the ceremony itself needs a real authenticator to exercise live)* |
...
| URL redirects | ✅ domain/path → target with 301/302/307/308; signed `redirect.apply` renders a **Caddy** snippet (one site block per source domain, whole-domain rules preserve the path via `{uri}`). Redirects card on the Domains page. Renderer + target validator unit-tested |
| DNSSEC | ✅ signed `dns.dnssec.enable` generates a KSK (ECDSAP256SHA256), signs the zone and returns the **DS record** to publish at the registrar; the DS is stored and shown on the Domains page, `dns.dnssec.disable` removes the keys. DS-record parser unit-tested *(live signing needs the BIND dnssec tools on the node)* |
| Dynamic DNS | ✅ token-updatable A records: `POST /ddns` mints a per-host update token; a **public** `GET /api/v1/ddns/update?token=…[&ip=…]` (DynDNS-style, no JWT — the token is the credential) points the host's A record at the caller's IP, upserts it (`SetARecord`, serial bumped) and re-applies the zone via a system-scoped (`superadmin`, org-bound) dispatch. Dynamic-DNS card on the Domains page |
| Directory privacy | ✅ password-protect a path with HTTP basic-auth; the control plane **bcrypt-hashes** the password (never stored in plaintext) and signed `protection.apply` renders a Caddy `basic_auth` snippet (grouped per domain, bcrypt-only). Directory-privacy card on the Domains page. Renderer unit-tested |
| Hotlink protection | ✅ block other sites from embedding your assets: the same `protection.apply` job renders a Caddy referer-guarded matcher (asset extensions + `not header_regexp Referer …` allowing the domain + extra referers) → `respond 403`; basic-auth and hotlink merge into one site block per domain. Hotlink card on the Domains page. Renderer unit-tested |
| Web Disk (WebDAV) | ✅ serve a folder over WebDAV behind HTTP basic-auth (bcrypt, control-plane-hashed) — the same `protection.apply` job renders a Caddy `webdav` block scoped to a path, merged into the per-domain site block alongside basic-auth/hotlink. Web Disk card on the Domains page. Renderer unit-tested |
| Managed databases (Postgres/MySQL/MariaDB/Redis/Mongo) | ✅ API → RBAC+OPA → envelope-encrypted credentials → signed job → hardened-container executor (runs live on a Docker node). Built-in **SQL query runner** (`database.query`, gated by its own permission): runs a statement inside the container with a statement-timeout, returns columns+rows (capped) — a phpMyAdmin alternative, with a **table browser** (list tables → click to page through rows) built on the same primitive. **Remote access** (cPanel "Remote MySQL"): allow external hosts by IP/CIDR → `database.access.apply` renders the Postgres `pg_hba.conf` block (SCRAM auth) and reloads, **or** for MySQL/MariaDB runs host-scoped `CREATE USER`/`GRANT` (the DB password is unsealed and travels only in the signed job; injection-safe host validation). **Export**: `database.dump` runs `pg_dump`/`mysqldump` in the container, gzips and uploads off-site to S3. CSV/TSV result + `pg_hba` + dump-args renderers unit-tested |
...
| Calendars & Contacts (CalDAV/CardDAV) | ✅ `caldav.ensure` launches a **Radicale** container; accounts are managed in the panel and `caldav.user.apply` renders the Radicale htpasswd users file (control-plane bcrypt). Card on the Email page with the client URL. Renderer + container args unit-tested |
...
| Spam management (Rspamd) | ✅ tune scoring thresholds (reject / flag), greylisting toggle, and **sender allow/deny lists**; signed `mail.spam.apply` renders Rspamd `actions.conf`, `greylist.conf` and an allow/deny `multimap` (+ map files). Config renderers unit-tested. Spam-filter card on the Email page |
| Hardening | ✅ custom-cert upload (`cert.install`), off-site **S3 backup** upload (aws CLI), private keys redacted from persisted jobs |
| Deploy from Git | ✅ `app.deploy` executor: git clone → docker build → hardened run (prior image retained for rollback). **Buildpacks**: a repo with no Dockerfile is auto-detected (Node/PHP/static from its marker files) and a hardened Dockerfile is synthesized before build. Detection + generation unit-tested |
| Backups & restore | ✅ API → signed `backup.create`/`backup.restore` jobs → agent tars/untars the target, uploads off-site to **S3** (aws CLI) and returns a **SHA-256 checksum**; the job-status callback finalizes the backup row (size + checksum + completed). **Recurring schedules** (daily/weekly + retention) fire from a 5-min control-plane runner; Backups UI shows checksums + a schedules section |
...
| WAF (application layer) | ✅ CRUD → signed `waf.apply` → agent renders a **Caddy** snippet (named matchers → `respond 403`) blocking by path regex, User-Agent regex or IP/CIDR; WAF UI with preset rules (WordPress probes, `.env`/`.git`, sqlmap). `render_caddy_waf` unit-tested |
...
| Customer webhooks (Pro) | ✅ outbound HTTPS callbacks for platform events (site/deploy/backup/invoice/health/security); each delivery signed `HMAC-SHA256` in `X-AsterPanel-Signature` (+ timestamp header). Subscribe to specific events or all; `POST /webhooks`, `/test`; one-time secret on create. Fires natively from `site.created`/`invoice.paid`. Sign/verify unit-tested |
...
| Web Panel — full hosting UI | ✅ every screen wired to a **real backend** + typed API client: sites, domains/DNS (+ nameservers, redirects, directory privacy), SSL, databases (+ SQL query runner), email + webmail + DKIM + forwarders/autoresponders/filters, FTP, file manager, cron, backups (+ schedules), runtime, **analytics**, one-click apps, metrics, health, logs, firewall, WAF, audit, API tokens + passkeys, notifications, plus the commercial layer (reseller, white-label, webhooks, billing, migrations) gated by edition |
| Internationalization (i18n) | ✅ lightweight provider keyed by source string (**English + Italian**), browser-locale auto-detection + a language switcher persisted to `localStorage`; sidebar/chrome + login localized, page bodies adopt `t()` incrementally |
| Node metrics (real) | ✅ agent samples CPU (`/proc/stat`), memory (`/proc/meminfo`), disk (`df`) and load every 15s → pushes to the CP metrics-ingest endpoint → `node_metrics` time series → `GET /metrics` aggregates the fleet (latest per node + CPU sparkline) for the panel (parsers unit-tested) |
| Web analytics (per domain) | ✅ signed `analytics.compute` → agent aggregates the site's Caddy JSON access log into **requests, unique visitors, bandwidth, top pages and a status-class breakdown**; Analytics page with a site picker. Access-log parser unit-tested |
...
| Service manager | ✅ signed `service.control` lists the node's AsterPanel containers (`docker ps`, parsed) and **restarts** a container — hard-scoped to `astp_*` names on both the control plane and the agent so shared infra can't be touched; gated by a dedicated `service.read`/`service.manage` RBAC pair. Services card on the Health page. `docker ps` parser unit-tested |
