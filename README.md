...
| Health checks & alerting | ✅ signed `health.check` → agent probes container liveness (`docker inspect`) + optional HTTP → status stored per site; `GET /health` + on-demand `POST /sites/{id}/health/check` power the Health panel; a notification fires on the transition into **down** (background sweep scheduler is the next step) |
