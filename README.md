...
| **Account & team** | Plans, invoices (PDF) and payments, support tickets, profile, password reset. **Team members** by email invitation with roles (administrator · developer · billing), developers optionally limited to specific services, and an account switcher for people who work on several accounts. |
...
Off-site (S3) backup storage · visitor analytics · code-level (PHP) tracing · a real multi-region CDN · SSH/SFTP access · database external access · CDN · buildpacks for apps without a Dockerfile · node draining and workload migration · REST API · domain registrar modules · e-invoicing (SDI).
