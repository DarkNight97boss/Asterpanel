...
Off-site (S3) backup storage · visitor analytics · code-level (PHP) tracing · a real multi-region CDN · SSH/SFTP access · database external access · CDN · file uploads in the file manager · per-site roles · SSH keys for SFTP · buildpacks for apps without a Dockerfile · node draining and workload migration · REST API · 2FA · domain registrar modules · e-invoicing (SDI).
