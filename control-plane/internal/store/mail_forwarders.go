...
// UpdateForwarder replaces a forwarder's destination set (source is the key and
// stays fixed). Org-scoped.
func (s *Store) UpdateForwarder(ctx context.Context, orgID, id uuid.UUID, destinations []string) (*MailForwarder, error) {
	const q = `
		UPDATE mail_forwarders SET destinations = $3
		WHERE id = $1 AND organization_id = $2
		RETURNING id, organization_id, source, destinations, is_catchall, created_at`
	return scanForwarder(s.pool.QueryRow(ctx, q, id, orgID, destinations))
}
